...
var beyond_inited = false;

function beyondInit() {
    //初始化设置按钮
    initEach($('#btn-setting'), function (element) {
    ...
    //初始化全屏按钮
    initEach($('#fullscreen-toggler'), function (currentNode) {
        currentNode.on('click', function (e) {
    ...
    //初始化popover
    initEach($('[data-toggle=popover]'), function (currentNode) {
        currentNode.popover({
            html: true,
    ...
    initEach($('[data-toggle=popover-hover]'), function (currentNode) {
        currentNode.popover({
            html: true,
            template: '<div class="popover ' + $(this)
                .data("class") +
    ...
    //初始化widget
    initEach($('.widget-buttons *[data-toggle="maximize"]'), function (currentNode) {
    ...
    initEach($('.widget-buttons *[data-toggle="collapse"]'), function (currentNode) {
        currentNode.on("click", function (event) {
            event.preventDefault();
            var widget = $(this).parents(".widget").eq(0);
    ...
    initEach($('.widget-buttons *[data-toggle="dispose"]'), function (currentNode) {
        currentNode.on("click", function (event) {
            event.preventDefault();
    ...
    initEach($("[data-toggle=tooltip]"), function (currentNode) {
        currentNode.tooltip({
            html: true
        });
    });
    ...
    initEach($(".sidebar-toggler"), function (currentNode) {
    ...
    initEach($("#sidebar-collapse"), function (currentNode) {
        currentNode.on('click', function () {
    ...
    initEach($(".sidebar-menu"), function (currentNode) {
        currentNode.on('click', function (e) {
    ...
    initEach($('#checkbox_fixednavbar'), function (currentNode) {
        currentNode.change(function () {
            $('.navbar')
    ...
    initEach($('#checkbox_fixedsidebar'),function(currentNode){
        currentNode .change(function () {
    ...
    initEach($('#checkbox_fixedbreadcrumbs'), function (currentNode) {
        currentNode.change(function () {
    ...
    initEach($('#checkbox_fixedheader'), function (currentNode) {
        currentNode.change(function () {
    ...
    initEach($("#skin-changer li a"),function(currentNode){
...
function initEach($elements, callback) {
    $elements.each(function () {
        var $element = $(this);
        if (!$element.attr('be-inited')) {
            callback($element);
        }
        $element.attr('be-inited', true);
    })
}